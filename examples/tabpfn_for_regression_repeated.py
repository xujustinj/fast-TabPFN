...
from tabpfn.profiling import timer, timing_summary
...
if __name__ == "__main__":
    ...
    for _ in range(args.trials):
        trial_output = list(main(fit_mode=fit_mode))
        if output is None:
            output = trial_output
        else:
    ...
    with (base_path / f"tabpfn_for_regression_repeated_{fit_mode}.txt").open("w") as f:
        for line in output:
            f.write(line + "\n")

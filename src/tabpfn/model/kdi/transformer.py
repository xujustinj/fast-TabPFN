...
class KDITransformer(TransformerMixin, BaseEstimator):
    ...
    def __init__(
        ...
        *,
        alpha: float = 1.0,
        kernel: Literal["polyexp", "gaussian"] = "polyexp",
        polyexp_order: int = 4,
        polyexp_eval: Literal["uniform", "train", "auto"] = "auto",
        n_quantiles: int | None = 1000,
        ...
        self.polyexp_eval: Literal["uniform", "train", "auto"] = polyexp_eval
        ...
        self.n_quantiles_: int | None = None
        self.subsample_ = None
        self.references_: np.ndarray | None = None
        self.quantiles_: np.ndarray | None = None
        self.n_features_in_ = None
        self.X_: np.ndarray | None = None

    def _gaussian_dense_fit(
        self,
        X: np.ndarray,
        ...
        for col, alpha in zip(X.T, alphas):
            ...
            if np.var(col) == 0:
                ...
                assert isinstance(self.references_, np.ndarray)
                ...
                inverse_func = spip.interp1d(
                    ...
                    col,
                    bounds_error=False,
                    fill_value=(xmin, xmax),  # type: ignore
                )
                quantiles = inverse_func(self.references_)
            quantiles_.append(quantiles)
        self.quantiles_ = np.transpose(quantiles_)
    ...
    def _polyexp_dense_fit(
        ...
        if self.polyexp_eval == "uniform":
            ...
            if self.n_quantiles_ is None:
                n_eval = n_samples
            else:
                assert self.n_quantiles_ is not None
                n_eval = self.n_quantiles_ + self.n_quantiles_
        else:
        ...
        *,
        ...
        with np.errstate(invalid="ignore"):  # hide NaN comparison warnings
            if output_distribution == "normal":
                ...
                raise ValueError(
                    f"Unexpected output_distribution: {output_distribution}"
                )
    ...
    def _transform(self, X: np.ndarray, *, inverse: bool = False) -> np.ndarray:
        ...
        X : ndarray of shape (n_samples, n_features)
            Projected data.
        """
        assert self.quantiles_ is not None
    ...
    def _transform_exact(self, X: np.ndarray) -> np.ndarray:
